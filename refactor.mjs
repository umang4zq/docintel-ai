...
const cssReplacements = [
  ...
  [/bg-white\/5(?![0-9])/g, "bg-zinc-100 dark:bg-white/5"],
...
function processDirectory(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      // Need to avoid replacing dark:text-white into dark:text-zinc-900 dark:text-white
      // So we will first remove dark: classes and then reapply, or we just do it carefully.
      // Easiest is to replace, then fix double replacements.
      for (const [regex, replacement] of cssReplacements) {
        content = content.replace(regex, replacement);
      }
      
      // Fixes for common bad double replacements:
      content = content.replaceAll('text-zinc-900 dark:text-zinc-900 dark:text-white', 'text-zinc-900 dark:text-white');
      content = content.replaceAll('dark:text-zinc-900 dark:text-white', 'dark:text-white');
      content = content.replaceAll('dark:border-zinc-200 dark:border-white/10', 'dark:border-white/10');
      content = content.replaceAll('dark:border-zinc-200 dark:border-white/8', 'dark:border-white/8');
      content = content.replaceAll('dark:border-zinc-300 dark:border-white/20', 'dark:border-white/20');
      content = content.replaceAll('dark:border-zinc-300 dark:border-white/30', 'dark:border-white/30');
      content = content.replaceAll('dark:bg-zinc-100 dark:bg-white/5', 'dark:bg-white/5');
      content = content.replaceAll('dark:bg-zinc-200 dark:bg-white/10', 'dark:bg-white/10');
      content = content.replaceAll('dark:bg-zinc-300 dark:bg-white/20', 'dark:bg-white/20');
      content = content.replaceAll('dark:bg-zinc-50 dark:bg-[#080808]', 'dark:bg-[#080808]');
      content = content.replaceAll('dark:bg-white dark:bg-[#0D1117]', 'dark:bg-[#0D1117]');
